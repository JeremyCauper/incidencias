...
class SubProblema extends Controller
{
    protected $filePath = 'config/jsons/sub_problema.json';
    ...
    {
        $registro = collect($this->all())->select('id', 'codigo_problema', 'descripcion', 'prioridad', 'eliminado', 'estatus')->firstWhere('id', $id);
        if (empty($registro)) {
            return $this->message(message: "El tipo de area buscado no exite.", status: 404);
        }
        return $registro;
    }

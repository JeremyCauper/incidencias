...
{
    protected $filePath = 'config/jsons/tipo_usuario.json';
    ...
    {
        $registro = collect($this->all())->select('id', 'descripcion', 'color', 'estatus')->firstWhere('id', $id);
        if (empty($registro)) {
            return $this->message(message: "El tipo usuario buscado no exite.", status: 404);
        }
        return $registro;
    }
